...
radar_config_file = './clutter_maps.json' # change this for for desired JSON file 
...
if polarization == 'horizontal' and scantype == 'ppi':
    for f in glob.glob(
        os.path.join(cluttermap_dir, "cluttermap_" + scantype + "_" + site + inst + "_*.nc")
    ):
        print(f)
    ...
    for f in glob.glob(
        ...
        print(f)
        ClutterMaskH, ClutterMaskV, ClutterPCTH, ClutterPCTV = get_pct_on_clutter_map_ppi(f,polarization)
        # Append output from each HSRHI file to lists
        clutter_mask_h.append(ClutterMaskH)
