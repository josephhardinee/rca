...
if __name__ == "__main__":
    ...
    if inst == "kasacr":  # KaSACR is H polarization only
        for f in glob.glob(
            os.path.join(cluttermapdir, "cluttermap_hsrhi_" + site + inst + "_*.nc")
        ):
            print(f)
        ...
        dataset = Dataset(
            outputdir + "cluttermap_hsrhi_" + site + inst + "_composite.nc",
            "w",
        ...
        for f in glob.glob(
            ...
            print(f)
            ClutterMaskH, ClutterMaskV, ClutterPCTH, ClutterPCTV = pct_on_clutter_map_hsrhi(
                f, clutter_v=True
            )
            # Append output from each HSRHI file to lists
