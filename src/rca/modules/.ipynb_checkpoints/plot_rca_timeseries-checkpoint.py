...
def plot_rca_timeseries_oneradar(
    rca_file, output_directory, baseline_date, polarization, scan_type
):
    """
    plot_rca_timeseries
    ...
    params = {"mathtext.default": "regular", "family": "sans", "size": 11}
    ...
    ax.scatter(df["DATE"], df["RCA_H"], color="k", linewidth=lw)
    ...
    if polarization == "dual":
        ...
        fig, ax = plt.subplots(figsize=[8, 4])
        ax.axhline(0.0, linestyle="--", color="grey")
        ax.scatter(df["DATE"], df["RCA_V"], color="k", linewidth=lw)
