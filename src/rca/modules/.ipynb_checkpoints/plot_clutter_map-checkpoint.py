...
def plot_clutter_map(
    ...
    if scan_type == "ppi":
        ...
        x = r * np.sin(az * np.pi / 180.0)
        y = r * np.cos(az * np.pi / 180.0)
        ...
        if polarization == "horizontal":
            ...
            im = ax1.pcolormesh(x, y, clutter_map_pcts_h_ppi, cmap=cmap, norm=norm)
            ...
            im = ax1.pcolormesh(x, y, clutter_map_pcts_v_ppi, cmap=cmap, norm=norm)
            ax1.set_xticklabels(labs)
            ax1.set_yticklabels(labs)
            cbar = fig.colorbar(im, ax=ax1)
            cbar.ax.set_ylabel(cbar_label)
    ...
    # ----------------------------------------------------------------------------------------
        ...
        x_e = r_e * np.sin(el_e * np.pi / 180.0)
        y_e = r_e * np.cos(el_e * np.pi / 180.0)
        x_w = r_w * np.sin(el_w * np.pi / 180.0)
        y_w = r_w * np.cos(el_w * np.pi / 180.0)
        ...
        if polarization == "horizontal":
            ...
            fig, axes = plt.subplots(
                nrows=6, ncols=2, sharex=True, figsize=[16, 12], constrained_layout=True
            )
