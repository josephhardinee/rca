...
csvfile = csvpath+'daily_rcavalues_ppi_'+site+inst+'.csv'
#csvfile = csvpath+'daily_rcavalues_ppi_'+site+inst+'_falseerror.csv'
...
if inst == 'kasacr':
    ...
    plt.savefig(fig_outpath+'rca_h_ppi_'+site+inst+'.png')
    ...
    ax.plot(df['DATE'],df['RCA_V'],
                ...
                linewidth=lw)
    ax.set_ylabel('RCA value')
    ax.set_title('Daily RCA values ($Z_V$) at '+location+'\n PPI')
    ax.set_ylim(ylim)
    ax.scatter(baseline,0.0,marker='D',linewidth=lww,color='b')
